...
`[ContextMenu]` entries. Runs against the package's official no-network `FakeAdsGateway` (since
0.2.0, in the runtime assembly) in **manual mode** — **not** real AppLovin MAX — because the Unity
Editor cannot display real MAX ads (no fill, no native views). The same gateway, left in its default
auto mode, is what a game wires behind `#if UNITY_EDITOR || TK_TEST_BUILD` to run its whole ads flow
without MAX.

## Why a fake gateway instead of MaxAdsGateway
...
the API. `FakeAdsGateway` auto-succeeds everything a real network would otherwise gate on:
...
- Every `Show*` call raises `Displayed`, then — because this sample sets `AutoResolveShows = false`
  — waits: the ad stays "up" until you tell the gateway how it resolved, via the methods below
  (wired to the component's `[ContextMenu]` entries). In the default auto mode the show would
  resolve itself synchronously instead (Displayed → RewardReceived → Hidden).
...
Because the fake gateway resolves loads synchronously, ads are ready right after `Initialized` fires
...
`AdsOptions.RevenueReporter`, though nothing raises `RevenuePaid` on its own (there's no real ad
network paying anything in the Editor; call `FakeAdsGateway.RaiseRevenue` yourself to exercise the
path) — see the `IntegrationExamples` sample's
`FirebaseAdRevenueReporterExample` for what a real revenue-reporting implementation looks like.

## Testing against real MAX ads

`FakeAdsGateway` is for API/flow familiarity only — it proves nothing about real ad delivery. To
