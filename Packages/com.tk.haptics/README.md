...
- **The throttle is a spam guard, not an effect scheduler.** A phone has one vibrator: a new effect *supersedes* the one still playing (`cancelled_superseded` in dumpsys). A predefined tick runs ~76 ms on real hardware, so a rapid stream that passes the 30 ms throttle (e.g. drag-boundary impacts every ~50–70 ms) plays each effect only partially — which often *feels* right for a drag. If you want every effect to complete during such a stream, raise `HapticThrottleSeconds` to at least the effect length (~0.08 s) for that interaction.
