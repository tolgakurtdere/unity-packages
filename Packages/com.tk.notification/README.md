...
| Seam + backend | `Runtime/Seams/`, `Runtime/Backends/` | `INotificationBackend` — the single interface a backend implements (must not throw). Ships `UnityMobileNotificationBackend`, the real Android/iOS impl that no-ops on every non-mobile target. A `FakeNotificationBackend` drives the tests. |
...
targets every one of these calls is a safe no-op (see **Non-mobile targets**), so you never guard the
call site with `#if`.
...
`UnityMobileNotificationBackend` reports `IsSupported == false` in the Editor and on desktop/console
targets, and **every operation becomes a safe no-op**: `Schedule` returns `null`, `ScheduleAll` /
`Cancel` / `CancelAll` / `RegisterChannel` / `OpenNotificationSettings` do nothing,
`IsPermissionGranted` and `RequestPermissionAsync` return `false`, and `TryGetLaunchNotification`
returns `false`. Nothing throws. This means you write the notification flow once and call it
unconditionally — no `#if UNITY_ANDROID` at your call sites, and Play-mode in the Editor never spams
the console or requires a device.
