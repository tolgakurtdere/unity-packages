...
> **Outcome (2026-07-23, post-approval): decision 2 was implemented, measured on device, and dropped.** The flag was built correctly on all three usages (`getFlags()` read back `2`) and arrived stripped — `dumpsys` showed `Flags=0` everywhere and nothing vibrated with the setting off. The open risk this spec flagged resolved against us, and not in the way predicted: the platform doesn't strip the bit universally (a reference app achieved `Flags=2` on the same device, same session, with no privileged permission), but no *public* route reaches it — `VibrationAttributes` exposes only `FLAG_BYPASS_INTERRUPTION_POLICY`, and the `AudioAttributes` route masks non-public bits in its own builder. Since an app can't detect the stripping at runtime, shipping the switch would have meant a public API that silently does nothing, so it was removed before tagging. **Decisions 1 and 3 shipped as designed.** The product decision behind 2 stands; the mechanism doesn't exist, and re-adding later would be additive.
