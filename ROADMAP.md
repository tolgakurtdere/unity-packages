...
| `com.tk.haptics` | 0.1.1 | Cross-platform haptic feedback: `Impact` / `Selection` / `Notification` with an enable toggle + per-type throttling; native iOS (Taptic) + Android (Vibrator) backends behind a testable seam, no-op fallback; standalone (no `com.tk.*` deps), no scoped registries. Device-verified on Android 14: merged VIBRATE permission, all three haptic types completing, and the Enabled gate stopping calls before the backend |
